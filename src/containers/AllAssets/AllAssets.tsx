  ...
  useEffect(() => {
    setUserGeneralInfo(
      accountPkh && userAssetsData && userAssetsData.user.length
        ? {
          maxCollateral: new BigNumber(userAssetsData.user[0].maxCollateral) ?? new BigNumber(0),
          outstandingBorrow: new BigNumber(userAssetsData.user[0].outstandingBorrow) ?? new BigNumber(0),
        }
        : null,
    );
