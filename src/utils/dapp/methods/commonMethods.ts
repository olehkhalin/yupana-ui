...
import { getAllowance } from "utils/dapp/helpers";
...
export const commonMethods = async (
  ...
  const preparedOtherYTokens =
    otherYTokens && otherYTokens.length
  ...
  const updateAllInterestMethods =
    preparedOtherYTokens && preparedOtherYTokens.length > 0
  ...
  if (isAllowanceNeeded && amount && tokenAddress) {
    ...
    if (tokenIsFA2) {
      ...
      const removeOperatorMethod = tokenContract.methods.update_operators([
        {
          remove_operator: {
            owner: accountPkh,
            operator: fabricaContractAddress,
      ...
      const resetApproveMethod = tokenContract.methods.approve(
        fabricaContractAddress,
        0
      );
